...
#define PORT 6980 // set the default port to 6980
...
// this function create the directory stext on reciveing signal 
void handleFileTransfer(int new_socket, char *filename, char *dest_path) {
    ...
    while (bytes_received < file_size) {
        ...
        if (write(dest_fd, buffer, bytes_read) < 0) {
            printf("Error writing file content to disk");
            break;
        }
...
// function get the rmfile command and it dleete from the directory the txt file 
...
// this function list all the txt file in stxt and send to smain 
void ListingFiles(int client_socket, const char *pathname, const char *filetype) {
    char command[1024];
    ...
    while (fgets(buffer, sizeof(buffer), fp) != NULL) {
        ...
        send(client_socket, buffer, strlen(buffer), 0);  // Send each line to the client
    }
    ...
    printf("End of list marker sent.\n");  // Debug output
}
...
void handle_request(int new_socket) {
    ...
    printf("Text Server received data: %s\n", buffer);

    // Determine whether the request is to receive, send, delete a file, or list files
    ...
    } else if (strncmp(buffer, "REQUEST_TXT_TAR;", 16) == 0) {
        tarCreation("txt.tar", ".txt", "~/stext");
        handleFileRequest(new_socket, "txt.tar");
        remove("txt.tar");
    }else if (strncmp(buffer, "LIST_FILES;", 11) == 0) {
    char *pathname = buffer + 11;
    printf(buffer , "pathname");
    printf("Searching for .txt files in: %s\n", pathname);
    ListingFiles(new_socket, pathname, ".txt");  // Pass ".txt" to list text files
}
     else {
        // Assume it's a file transfer if not a file request, deletion, or list files command
